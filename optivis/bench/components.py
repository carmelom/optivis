...
class AbstractComponent(with_metaclass(abc.ABCMeta, optivis.bench.AbstractBenchItem)):
...
class Source(with_metaclass(abc.ABCMeta, AbstractComponent)):
...
class Mirror(with_metaclass(abc.ABCMeta, AbstractComponent)):
    def __init__(self, *args, **kwargs):
...
class Lens(with_metaclass(abc.ABCMeta, AbstractComponent)):
...
class Plate(with_metaclass(abc.ABCMeta, AbstractComponent)):
    def __init__(self, aoi, *args, **kwargs):
        inputNodes = [
          # input node azimuth defined WRT input light direction
          nodes.InputNode(name="fr", component=self, position=optivis.geometry.Coordinates(0.5, 0), aoiMultiplier=-1, aoiOffset=180),
...
class Modulator(with_metaclass(abc.ABCMeta, AbstractComponent)):
    def __init__(self, aoi, *args, **kwargs):
        inputNodes = [
          # input node azimuth defined WRT input light direction
          nodes.InputNode(name="fr", component=self, position=optivis.geometry.Coordinates(0.5, 0), aoiMultiplier=-1, aoiOffset=180),
...
class Sink(with_metaclass(abc.ABCMeta, AbstractComponent)):
